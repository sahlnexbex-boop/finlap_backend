...
exports.startReminderScheduler = exports.checkAndProcessDueReminders = exports.ensureInAppNotificationRecord = exports.scheduleReminderNotification = exports.parseReminderDateTime = void 0;
...
exports.parseReminderDateTime = parseReminderDateTime;
/**
...
const scheduleReminderNotification = async (reminder) => {
    if (!reminder || !reminder.userId)
        return;
    const scheduledDate = (0, exports.parseReminderDateTime)(reminder.date, reminder.time);
    ...
    if (!scheduledDate || scheduledDate.getTime() <= Date.now() + 1000) {
        await (0, exports.ensureInAppNotificationRecord)(reminder, scheduledDate || new Date());
    }
};
exports.scheduleReminderNotification = scheduleReminderNotification;
/**
 * Creates in-app Notification DB record & sends FCM push notification if user has registered FCM token
 */
const ensureInAppNotificationRecord = async (reminder, scheduledAt) => {
    try {
        ...
        if (!existing) {
            ...
            if (user && user.notifications && user.fcmToken) {
                ...
                await (0, firebaseService_1.sendFcmPushNotification)({
...
exports.ensureInAppNotificationRecord = ensureInAppNotificationRecord;
/**
 * Background worker checking due reminders periodically
 */
const checkAndProcessDueReminders = async () => {
    try {
        const now = new Date();
        ...
        const dueReminders = await db_1.prisma.reminder.findMany({
            where: {
                date: { lte: todayStr },
                status: 'PENDING',
            },
        ...
        for (const reminder of dueReminders) {
            const scheduledDateTime = (0, exports.parseReminderDateTime)(reminder.date, reminder.time);
            if (scheduledDateTime && scheduledDateTime.getTime() <= now.getTime() + 1000) {
                await (0, exports.ensureInAppNotificationRecord)(reminder, scheduledDateTime);
            }
        }
    }
    catch (error) {
...
exports.checkAndProcessDueReminders = checkAndProcessDueReminders;
/**
...
const startReminderScheduler = () => {
    ...
    // Initial check
    (0, exports.checkAndProcessDueReminders)();
