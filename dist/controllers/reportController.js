...
exports.exportReport = exports.getReport = exports.getReportData = void 0;
const db_1 = require("../db");
const userController_1 = require("./userController");
const normalizeTransactionType = (type, amount) => {
    if (type === 1 || type === '1' || type === 'INCOME')
        return 'INCOME';
    if (type === 0 || type === '0' || type === 'EXPENSE')
        return 'EXPENSE';
    return amount !== undefined && amount > 0 ? 'INCOME' : 'EXPENSE';
};
const getReportData = async (userId, queryParams) => {
    ...
    transactions.forEach((t) => {
        ...
        if (tType === 'INCOME')
            categoryMap[cat].income += absAmt;
        else
            categoryMap[cat].expense += absAmt;
        categoryMap[cat].count += 1;
        // Account breakdown
        const acc = t.fundingSource || t.walletName || 'Default Account';
        if (!accountMap[acc])
            accountMap[acc] = { income: 0, expense: 0, count: 0 };
        if (tType === 'INCOME')
            accountMap[acc].income += absAmt;
        else
            accountMap[acc].expense += absAmt;
        ...
        if (!entityMap[entId])
            entityMap[entId] = { name: entName, income: 0, expense: 0, count: 0 };
        if (tType === 'INCOME')
            entityMap[entId].income += absAmt;
        else
            entityMap[entId].expense += absAmt;
...
const getReport = async (req, res) => {
    try {
        ...
        const reportData = await (0, exports.getReportData)(userId, req.query);
        res.json({
            success: true,
            data: reportData,
        });
